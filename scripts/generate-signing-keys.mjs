 ...
 *
 * O que ele faz:
 *   1. Roda `tauri signer generate` e salva o par em .tauri/duck-pomodoro.key(.pub)
 ...
 *   - TAURI_SIGNING_PRIVATE_KEY         → conteúdo de .tauri/duck-pomodoro.key
 *   - TAURI_SIGNING_PRIVATE_KEY_PASSWORD → a senha usada aqui
 *
 * A chave PÚBLICA vai commitada no tauri.conf.json — ela não é secreta.
 * A chave PRIVADA (.tauri/duck-pomodoro.key) NÃO deve ser commitada.
 */
...
const keyPath = join(keyDir, "duck-pomodoro.key");
...
const gen = spawnSync(npxCmd, genArgs, {
  cwd: repoRoot,
  stdio: password ? "inherit" : ["inherit", "inherit", "inherit"],
});
...
const pubkeyLine = rawPub
  .split("\n")
  .map((l) => l.trim())
  .find((l) => /^RW[A-Za-z0-9+/=]+$/.test(l));

if (!pubkeyLine) {
  console.error(
    `❌ Conteúdo inesperado em ${pubKeyPath}. Esperava uma linha começando com RW...`
  );
  process.exit(1);
}
